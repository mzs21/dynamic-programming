...
const canSum = (targetSum, numbers, memo = {}) => {
    if (targetSum in memo)
        return memo[targetSum]; // Checking whether the 'targetSum' is in the 'memo' or not
    if (targetSum === 0)
        return true; // If the base case is 0, return true
    if (targetSum < 0)
        return false; // targetSum can't be negative
    for (let num of numbers) {
        let remainder = targetSum - num; // Branching through the tree
